@safetestset "Process New Patterns" begin
    ...
    chains = [Chain(1, [0.3, 0.3], [1, 2], 0.2), Chain(2, [0.3, 0.3], [1, 3], 0.2)]
    chains[1].acceptance[1] = false
    chains[2].acceptance[1] = false

    parms = [[0.3, 0.4], [0.3, 0.5]]
    options = Options(;
        radius = 0.2,
        bounds = [(0, 1), (0, 1)],
        n_iters = 100,
...
@safetestset "update_position!" begin
    ...
    pattern = [4, 3]
    bounds = [(0, 1)]
    chain = Chain(1, parms, pattern, 0.3)
    update_position!(chain, proposal, pattern, bounds)
    @test chain.parms == [0.3]
...
@safetestset "5D Polytope" begin
    ...
    # partition boundaries
    polytopes = [Polytope(rand(n_dims)) for i = 1:n_part]
    bounds = fill((0, 1), n_dims)
