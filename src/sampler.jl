...
function adapt!(
    ...
    if chain.level == 0
        ...
        if mod(n_attempt, n) == 0
            ...
            elseif (a_rate ≥ (t_rate - 0.08)) && (a_rate < (t_rate + 0.16))
                chain.level = 1
                chain.n_attempt = 0
            elseif a_rate ≥ (t_rate + 0.16)
                if λ < 0
                    λ += 0.5
                    chain.level = 1
                    chain.n_attempt = 0
    ...
    elseif chain.level == 1
        ...
        if mod(n_attempt, n) == 0
            a_rate = chain.n_accept / n
            chain.n_accept = 0
            if a_rate < (t_rate - 0.05)
                λ = λ - 0.25 / ceil(v / 2)
                if v == 4
                    chain.level = 2
                    chain.n_attempt = 0
                end
            elseif (a_rate ≥ (t_rate - 0.05)) && (a_rate < (t_rate - 0.01))
                λ -= 0.125
                chain.level = 2
                chain.n_attempt = 0
            elseif (a_rate ≥ (t_rate - 0.01)) && (a_rate < (t_rate + 0.04))
                chain.level = 2
                chain.n_attempt = 0
            elseif (a_rate ≥ (t_rate + 0.04)) && (a_rate < (t_rate + 0.10))
                λ += 0.125
                chain.level = 2
                chain.n_attempt = 0
            elseif a_rate > (t_rate + 0.10)
                λ = λ + 0.25 / ceil(v / 2)
                if v == 4
                    chain.level = 2
                    chain.n_attempt = 0
